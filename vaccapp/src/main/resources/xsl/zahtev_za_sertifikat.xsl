...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:z="http://tim.robot/zahtev_za_sertifikat"
     version="2.0">
    <xsl:template match="/">
        <html>
            <head>
            ...
            <body>
                ...
                <p class="indent-paragraph">Ime i prezime:
                    <b>
                        <xsl:value-of select="concat(' ', //z:Ime, ' ', //z:Prezime)"/>
                    </b>
                </p>
                <p class="indent-paragraph">Datum rodjenja:
                    <b>
                        <xsl:value-of select="concat(' ', //z:Datum_rodjenja)"/>
                    </b>
                </p>
                <p class="indent-paragraph">Pol:
                    <b>
                        <xsl:value-of select="concat(' ', //z:Pol)"/>
                    </b>
                </p>
                <xsl:choose>
                    <xsl:when test="string-length(//z:JMBG/text()) = 13">
                        <p class="indent-paragraph">Jedinstveni maticni broj gradjanina:
                            <b>
                                <xsl:value-of select="concat(' ', //z:JMBG)"/>
                            </b>
                        </p>
                    </xsl:when>
                    <xsl:otherwise>
                        <p class="indent-paragraph">Broj pasosa:
                            <b>
                                <xsl:value-of select="concat(' ', //z:Broj_pasosa)"/>
                            </b>
                        </p>
                    </xsl:otherwise>
                </xsl:choose>
                ...
                <p class="indent-paragraph">
                    <xsl:value-of select="//z:Razlog_za_podnosenje"/>
                </p>
                ...
                <p class="indent-paragraph">U
                    <b><xsl:value-of select="concat(' ', //z:Mesto)"/>,
                    </b>
                </p>
                <p class="indent-paragraph">Datum:
                    <b>
                        <xsl:value-of select="concat(' ', //z:Datum)"/>
                    </b>
                </p>
