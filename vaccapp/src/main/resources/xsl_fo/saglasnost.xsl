<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:s="http://tim.robot/obrazac_saglasnosti_za_imunizaciju"
                xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0">
    <xsl:template match="/">
        <fo:root>
            <fo:layout-master-set>
                <fo:simple-page-master
                        master-name="saglasnost"
                        page-height="29.7cm"
                        page-width="21cm"
                        margin-top="1.3cm"
                        margin-bottom="2cm"
                        margin-left="1.3cm"
                        margin-right="1.3cm">
                    <fo:region-body/>
                </fo:simple-page-master>
            </fo:layout-master-set>

            <fo:page-sequence master-reference="saglasnost">

                <fo:flow flow-name="xsl-region-body">
                    <fo:block-container>
                        <fo:block-container width="66%" left="0in" top="0in" position="absolute">
                            <fo:block font-family="sans-serif" font-size="19px" font-weight="bold">
                                SAGLASNOST ZA SPROVODJENJE
                                PREPORUCENE IMUNIZACIJE
                            </fo:block>
                        </fo:block-container>
                        <fo:block-container width="34%" left="66%" top="0in" position="absolute">
                            <fo:block text-align="right">
                                <fo:external-graphic
                                        src="url('data:image/jpeg;base64,')"/>
                            </fo:block>
                        </fo:block-container>
                    </fo:block-container>
                    <fo:block font-family="sans-serif" font-size="13px" margin-top="45px">
                        (popunjava pacijent)
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px" margin-top="10px">
                        <fo:inline font-weight="bold">
                            Drzavljanstvo
                        </fo:inline>
                        <fo:inline>
                            1) Republika Srbija | JMBG
                        </fo:inline>
                        <xsl:if test="string-length(//s:JMBG/text()) = 13">
                            <fo:inline font-weight="bold">
                                <xsl:value-of select="concat(' ', //s:JMBG)"/>
                            </fo:inline>
                        </xsl:if>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px" text-indent="85px">
                        <fo:inline>
                            2) Strano drz.
                        </fo:inline>
                        <xsl:if test="string-length(//s:JMBG/text()) = 9">
                            <fo:inline font-weight="bold">
                                <xsl:value-of select="concat(' ', //s:Naziv, ' ')"/>
                            </fo:inline>
                        </xsl:if>
                        | Br. pasosa
                        <xsl:if test="string-length(//s:JMBG/text()) = 9">
                            <fo:inline font-weight="bold">
                                <xsl:value-of select="concat(' ', //s:Br_pasosa, ' ')"/>
                            </fo:inline>
                        </xsl:if>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Prezime</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Podaci_o_pacijentu/s:Prezime, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">Ime</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Podaci_o_pacijentu/s:Ime, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">Ime roditelja</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Podaci_o_pacijentu/s:Ime_roditelja)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Pol:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Pol, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">Datum rodjenja</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Datum_rodjenja, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">Mesto rodjenja</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Mesto_rodjenja)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Adresa (ulica i broj)</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Ulica, ' ', //s:Broj, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">Mesto/Naselje</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Naselje)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Opstina/Grad</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Grad, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">Tel. fiksni</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Telefon_fiksni)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Tel. mobilni</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Telefon_mobilni, ' | ')"/>
                        </fo:inline>
                        <fo:inline font-weight="bold">email</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Imejl)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Radni status:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Radni_status)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Zanimanje zaposlenog:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ', //s:Zanimanje_zaposlenog)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Korisnik ustanove soc. zast.</fo:inline>
                        <xsl:if test="//s:Korisnik_socijalne_zastite/text() = 'DA'">
                            <fo:inline>
                                Da
                            </fo:inline>
                        </xsl:if>
                        <xsl:if test="//s:Korisnik_socijalne_zastite/text() = 'NE'">
                            <fo:inline>
                                Ne
                            </fo:inline>
                        </xsl:if>
                        <fo:inline font-weight="bold">
                            | Naziv i opstina sedista
                        </fo:inline>
                        <xsl:if test="//s:Sediste_socijalne_zastite">
                        <xsl:value-of select="concat(' ', //s:Sediste_socijalne_zastite/s:Naziv, ' ', //s:Opstina)"/>
                        </xsl:if>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px">
                        <fo:inline font-weight="bold">Izjavljujem da: <xsl:value-of select="concat(' ', //s:Saglasnost)"/>,</fo:inline>
                        <fo:inline>
                            sa sprovodjenjem aktivne/pasivne imunizacije (naziv imunoloskog leka):
                            <xsl:value-of select="concat(' ', //s:Naziv_imunoloskog_leka)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="12px" padding="5px" font-weight="bold">
                        Lekar mi je objasnio prednosti i rizike od sprovodjenja aktivne/pasivne imunizacije navedenim
                        imunoloskim lekom.
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="5px" padding="5px">
                        ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
                        ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
                        ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
                        ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
                    </fo:block>
                    <xsl:if test="//s:Evidencija_o_vakcinaciji">
                        <fo:block font-family="sans-serif" font-size="16px" padding="5px" text-align="center">EVIDENCIJA
                            O VAKCINACIJI PROTIV COVID-19
                        </fo:block>
                        <fo:block font-family="sans-serif" font-size="12px" margin-bottom="5px" text-align="center">
                            (popunjava zdravstevni radnik)
                        </fo:block>
                        <fo:block font-family="sans-serif" font-size="12px" margin-bottom="5px">
                            Zdravstvena ustanova
                            <fo:inline font-weight="bold">
                                <xsl:value-of select="concat(' ', //s:Zdravstvena_ustanova, ' ')"/>
                            </fo:inline>
                            Vakcinacijski punkt
                            <fo:inline font-weight="bold">
                                <xsl:value-of select="concat(' ', //s:Vakcinacijski_punkt)"/>
                            </fo:inline>
                        </fo:block>
                        <fo:block font-family="sans-serif" font-size="12px" margin-bottom="5px">
                            Ime, prezime, faksimil i br. telefona lekara:
                            <fo:inline font-weight="bold">
                                <xsl:value-of
                                    select="concat(' ', //s:Podaci_o_lekaru/s:Ime, ', ', //s:Podaci_o_lekaru/s:Prezime, ', ', //s:Broj_telefona)"/>
                            </fo:inline>
                        </fo:block>
                        <fo:block font-family="sans-serif" font-size="12px" margin-bottom="5px">
                            Pre davanja vakcine pregledati osobu i upoznati je sa koristima i o mogucim nezeljenim
                            reakcijama posle vakcinacije. Obavezno upisati svaku datu
                            vakcinu i sve trazene podatke u ovaj obrazac i podatke uneti u licni karton o izvrsenim
                            imunizacijama i zdravstveni karton.
                        </fo:block>
                        <fo:block font-size="10px">
                            <fo:table font-family="serif" margin="5px auto 5px auto" border="1px">
                                <fo:table-column column-width="18%" border="1px solid darkgrey"/>
                                <fo:table-column column-width="17%" border="1px solid darkgrey"/>
                                <fo:table-column column-width="15%" border="1px solid darkgrey"/>
                                <fo:table-column column-width="12%" border="1px solid darkgrey"/>
                                <fo:table-column column-width="10%" border="1px solid darkgrey"/>
                                <fo:table-column column-width="13%" border="1px solid darkgrey"/>
                                <fo:table-column column-width="15%" border="1px solid darkgrey"/>
                                <fo:table-body>
                                    <fo:table-row border="1px solid darkgrey">
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Naziv vakcine</fo:block>
                                        </fo:table-cell>
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Datum davanja vakcine</fo:block>
                                        </fo:table-cell>
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Ekstremitet</fo:block>
                                        </fo:table-cell>
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Serija vakcine (lot)</fo:block>
                                        </fo:table-cell>
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Proizvodjac</fo:block>
                                        </fo:table-cell>
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Nezeljena reakcija</fo:block>
                                        </fo:table-cell>
                                        <fo:table-cell font-family="sans-serif" padding="2px">
                                            <fo:block>Potpis lekara</fo:block>
                                        </fo:table-cell>
                                    </fo:table-row>
                                    <xsl:for-each select="//s:Primljena_vakcina">
                                        <fo:table-row border="1px solid darkgrey">
                                            <fo:table-cell padding="2px">
                                                <fo:block>
                                                    <xsl:value-of select="s:Naziv"/>
                                                </fo:block>
                                            </fo:table-cell>
                                            <fo:table-cell padding="2px">
                                                <fo:block>
                                                    <xsl:value-of select="s:Datum_izdavanja"/>
                                                </fo:block>
                                            </fo:table-cell>
                                            <fo:table-cell padding="2px">
                                                <fo:block>
                                                    <xsl:value-of select="s:Ekstremitet"/>
                                                </fo:block>
                                            </fo:table-cell>
                                            <fo:table-cell padding="2px">
                                                <fo:block>
                                                    <xsl:value-of select="s:Serija_vakcine"/>
                                                </fo:block>
                                            </fo:table-cell>
                                            <fo:table-cell padding="2px">
                                                <fo:block>
                                                    <xsl:value-of select="s:Proizvodjac"/>
                                                </fo:block>
                                            </fo:table-cell>
                                            <fo:table-cell padding="2px">
                                                <fo:block>
                                                    <xsl:value-of select="s:Nezeljena_reakcija"/>
                                                </fo:block>
                                            </fo:table-cell>
                                        </fo:table-row>
                                    </xsl:for-each>
                                </fo:table-body>
                            </fo:table>
                        </fo:block>
                        <fo:block font-family="sans-serif" font-size="12px" margin-bottom="5px">
                            <fo:inline font-weight="bold">Napomena:</fo:inline>
                            Obrazac se cuva kao deo medicinske dokumentacije pacijenta.
                        </fo:block>
                    </xsl:if>
                </fo:flow>
            </fo:page-sequence>
        </fo:root>
    </xsl:template>
</xsl:stylesheet>